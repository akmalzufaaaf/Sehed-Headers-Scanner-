# merapikan CSP policy dengan indentasi
def format_csp_policy(csp_resources):
    csp_directives = ["default-src 'self';"]  

    # Tambahkan direktif lainnya jika ada sumber yang ditentukan
...
def ensure_csp_exists(missing_headers, csp_resources):
    if "Content-Security-Policy" in missing_headers:
        if not csp_resources:
            csp_resources = {"default-src": ["'self'"]}  # set default-src 'self' jika tidak ada
...
#konfigurasi Nginx dan OpenResty
